defmodule EctoPaginatorTest do
  use EctoPaginator.DataCase
  doctest EctoPaginator

  alias Calendar.DateTime, as: DT

  alias EctoPaginator.Cursor
      ...
      %Page{entries: entries, metadata: metadata} =
        ...
        |> Repo.paginate(
          ...
          fetch_cursor_value_fun: fn
            ...
            schema, field ->
              EctoPaginator.default_fetch_cursor_value(schema, field)
          end
        )
  ...
  test "per-record cursor generation", %{
    ...
    assert EctoPaginator.cursor_for_record(p1, charged_at: :asc, id: :asc) ==
             encode_cursor([p1.charged_at, p1.id])

    assert EctoPaginator.cursor_for_record(p7, amount: :asc) == encode_cursor([p7.amount])
  end

  test "per-record cursor generation with custom cursor value function", %{
    payments: {p1, _p2, _p3, _p4, _p5, _p6, _p7, _p8, _p9, _p10, _p11, _p12}
  } do
    assert EctoPaginator.cursor_for_record(p1, [charged_at: :asc, id: :asc], fn schema, field ->
             case field do
